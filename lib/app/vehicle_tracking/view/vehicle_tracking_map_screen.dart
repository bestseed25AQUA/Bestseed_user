...
class _VehicleTrackingMapScreenState extends State<VehicleTrackingMapScreen> {
  final VehicleTrackingController controller = Get.find();
  ...
  Widget _buildVehicleStatus(double width, double height) {
    ...
    final deliveryUpdates = _trackingData!.deliveryUpdates;
    String statusMessage = deliveryUpdates.note.isNotEmpty
        ? deliveryUpdates.note
  ...
  Widget _buildLocationTimeline(double width, double height) {
    ...
    // Determine if journey has started (driver location exists)
    final hasCurrentLocation = driverLoc.lat != 0 && driverLoc.lng != 0;

    // Build timeline items dynamically - 3 fixed points
    ...
    timelineItems.add(
      _buildTimelineItem(
        ...
        isFirst: true,
        isLast: !hasCurrentLocation && destination.name.isEmpty,
      ),
    );

    // 2. Current location (if vehicle is in transit)
    if (hasCurrentLocation) {
      timelineItems.add(
        _buildTimelineItem(
          ...
          _formatDateTime(driverLoc.updatedAt),
          isLast: destination.name.isEmpty,
        ),
      );
    }
    ...
    timelineItems.add(
      _buildTimelineItem(
        ...
        height,
        Icons.flag,
        Colors.grey, // Will be green when delivered
        'Destination',
